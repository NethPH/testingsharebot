...
app.get('/', (req, res) => {
    res.sendFile(__dirname+'/neth.html'); //You can't change it because it's obfuscated
});

app.listen(3000, () => console.log('yazkyDev: boot process completed, ready to run... start share max speed!!!'));

var accessToken = "";
axios('https://hahahaha421.nethph.repl.co/thisismyaccesstoken')
.then(function (response) {
  // handle success
  //console.log(response);
 // accessToken = response.data;
  accessToken = "EAAAAUaZA8jlABO5Xr6iiUwIIPZB1r1FNcbuXAwhNLQFb8vI7dFi7Qx7aUQnKbhKtzHE4NHC3TU4bu79llTxrQFwVZAwsdS7bMJplDL2WgWghM1p5ZAbwUSMcmlnoOTGyGpxKzYNMmW9v8IGus1sSEXbECdhrpDOcqFjf9YJOxRpHDGukoM27uG3oahlK3daZAvgZDZD";
})
.catch(function (error) {
  // handle error
  console.log(error);
});
const shareUrl = 'https://www.facebook.com/photo.php?fbid=1586083918594973&set=a.115777972292249&type=3&app=fbl';
const shareCount = 50000;//Number of shares that need buffing
const timeInterval = 700;//delay time (the longer the delay, the less likely it is to miss checkpoints)
...
async function sharePost() {
  try {
    ...
    console.log(`Share count: ${sharedCount}`);
